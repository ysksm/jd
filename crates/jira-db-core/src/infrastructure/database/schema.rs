...
impl Schema {
    ...
    fn run_migrations(conn: &Connection) -> DomainResult<()> {
        ...
        Self::add_column_if_not_exists(conn, "issues", "due_date", "DATE")?;
        // Migration: issue_snapshotsテーブルにdue_dateカラムを追加
        Self::add_column_if_not_exists(conn, "issue_snapshots", "due_date", "DATE")?;
        Ok(())
    }
